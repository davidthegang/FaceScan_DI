...
// --- CONSTANTS ---
const SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbzq6ne8SuyYpH5-pSr_xIgW2B_ziWzZTBBVkXqbD1Ehh5N4Yg39Bdzy9eGIEb6Wlsk/exec";
const FACE_MATCH_THRESHOLD = 0.5; // Stricter verification
const MODEL_URL = "./weights";
// REMOVED: No longer using an external proxy
// const PROXY_URL = '...';
...
function AppProvider({ children }) {
  ...
  const [color, setColor] = useState(
    () => localStorage.getItem("color") || "theme-indigo"
  );
...
const translations = {
  kh: {
    ...
    loginTitle: "ប្រព័ន្ធគ្រប់គ្រងទិន្នន័យរូបភាព",
  ...
  choice: (
    ...
    >
      <path
        ...
        strokeLinejoin="round"
        strokeWidth="2"
        d="M3 5h12M9 3v2m4 13-4-4M3 10h12M3 15h4M4 19h16a1 1 0 0 0 1-1V6a1 1 0 0 0-1-1H4a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1z"
      ></path>
    </svg>
  ),
};
...
function LoginScreen({ onLoginSuccess }) {
  ...
  // *** MODIFIED: Use our own Apps Script as a proxy ***
  const prepareReferenceFace = async (admin) => {
    setStatus(t("loadingRefFace"));
    try {
      // 1. Call our Google Script to fetch the image
      const response = await fetchData("getAdminImage", {
        url: admin.imageUrl,
      });

      // 2. The script returns a Base64 data URL
      const dataUrl = response.data;
      const referenceImage = await faceapi.fetchImage(dataUrl);

      // 3. Continue with face detection
      ...
      if (!detection) {
        setModal({
          type: "error",
          message: "Could not find face in reference image.",
          onClose: () => {
            setModal(null);
            handleBackToSelect();
          },
      ...
      setModal({
        type: "error",
        message: `Could not load reference image. ${e.message}`,
        onClose: () => {
          setModal(null);
          handleBackToSelect();
        },
  ...
  const getStatusColor = () => {
    if (
      ...
      status.includes("មិនត្រឹមត្រូវ") ||
      status.includes("not found")
    )
      return "text-red-400";
    if (status.includes("កំពុង") || status.includes("Loading"))
      return "text-yellow-400";
    if (status.includes("ស្វាគមន៍") || status.includes("Welcome"))
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-main">
      ...
      <div className="w-full max-w-md p-6 mx-auto border rounded-2xl bg-content border-main shadow-2xl">
        <div className="container w-full relative">
          ...
          />
          <h1 className="mb-2 text-2xl text-center khmer-title text-main">
            {t("loginTitle")}
          </h1>
          ...
          {view === "camera" && (
            <div className="flex flex-col items-center space-y-4">
              <div
                id="login-video-wrapper"
                className="video-container w-[90%] mx-auto rounded-full overflow-hidden"
              >
                <video
                  id="login-video"
                  ref={videoRef}
...
// *** NEW: Reusable Admin Image Component ***
// This component fetches the admin image using the Google Script proxy
// and handles loading/error states.
function AdminImage({ imageUrl, alt, className }) {
  const [imgSrc, setImgSrc] = useState(null); // Start with null to show loading
  ...
  useEffect(() => {
    ...
    // Fetch the image using our Google Script proxy
    fetchData("getAdminImage", { url: imageUrl })
      .then((response) => {
        setImgSrc(response.data); // This is the Base64 data URL
      })
      .catch((err) => {
        console.error("Failed to fetch admin image:", err);
        setError(true);
      });
  }, [imageUrl]); // Re-run when the image URL changes
  ...
  return (
    <aside className="flex flex-col w-64 h-full bg-sidebar z-40 border-r border-main">
      <div className="py-6 mb-4 text-center">
        ...
        />
        <h2 className="text-xl font-bold khmer-title text-main">
          ឧស្សាហកម្មឌីជីថល
        </h2>
      </div>
        ...
        {navItems.map((item) => (
          <li key={item.id}>
            <a
              ...
              className={`nav-link flex items-center gap-3 px-4 py-3 rounded-lg text-muted hover:text-main ${
  ...
  return (
    <header className="sticky top-0 z-20 flex items-center justify-between flex-shrink-0 w-full h-16 px-6 bg-content/80 backdrop-blur-lg border-b border-main">
      <div className="flex items-center">
        ...
        <h1 className="text-xl font-bold khmer-title text-main">
          {t("headerTitle")}
        </h1>
      </div>
      <div className="flex items-center">
        <button
          ...
          className="w-10 h-10 rounded-full overflow-hidden border-2 border-theme focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-theme"
        >
          <AdminImage
...
function PageScan({ students, onRefreshData }) {
    ...
    setModal({
      ...
      actions: {
        [t("autoCapture")]: {
          ...
          className:
            "w-full text-white py-3 rounded-lg font-semibold transition btn-theme",
        },
        [t("manualCapture")]: {
          callback: () => {
  ...
  }, [facingMode, cameraMode, stopStudentCamera, t]);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setModal({
        type: "error",
        message: `${t("uploadFailed")}: ${e.message}`,
        onClose: () => setModal(null),
      });
    }
  };
  ...
  const getStatusColor = () => {
    if (cameraStatus.includes(t("cameraStatusSuccess")))
      return "text-green-400";
  ...
  return (
    <div className="main-content rounded-2xl p-6 bg-content">
      ...
      <div className="container w-full max-w-md mx-auto relative">
        ...
        <h1 className="text-center text-2xl mb-2 khmer-title text-main">
          {t("scanTitle")}
        </h1>
        <p className="text-center text-muted mb-6 h-5">
          ...
          {view === "camera" &&
            (selectedStudent
              ? `${t("scanningFor")}: ${selectedStudent[1]}`
              : "...")}
              ...
              {filteredStudents.map((s) => {
                ...
                return (
                  <li
                    ...
                    className={`p-3 rounded-lg transition flex justify-between items-center ${
                      isCompleted
                        ? "opacity-70 text-green-400 font-semibold pointer-events-none"
                        : "cursor-pointer hover:bg-slate-700"
                    }`}
                  >
                    <span className={isCompleted ? "" : "text-main"}>
                      {id} - {name}
                    </span>
                    {isCompleted && (
                      <i className="fas fa-check-circle ml-2 text-green-400"></i>
                    )}
                  </li>
                );
              })}
        ...
        {view === "camera" && (
          <div className="flex flex-col items-center space-y-4">
            <div
              id="video-wrapper"
              className={`video-container w-[90%] mx-auto rounded-full overflow-hidden ${
                cameraStatus.includes(t("cameraStatusSuccess")) ? "ready" : ""
              }`}
            >
              <video
                id="video"
            ...
            ></canvas>
            <p
              className={`text-center text-sm h-5 font-semibold ${getStatusColor()}`}
            >
              {cameraStatus}
            </p>
          </div>
...
function PageList({ students, savedRecords }) {
  ...
  const imageUrlMap = useMemo(
    () => new Map(savedRecords.slice(1).map((r) => [r[2], r[5]])),
    [savedRecords]
  );
  const classes = useMemo(
    () => [...new Set(students.map((s) => s[2]).filter(Boolean))].sort(),
    [students]
  );
      ...
      <div className="table-container flex-1 overflow-auto">
        {students.length === 0 ? (
          <div className="text-center py-10">
            <p className="text-muted">{t("loadingStudents")}</p>
          </div>
        ) : (
                    ...
                    <td className="p-3">
                      {imageUrl ? (
                        <a
                          ...
                          className="relative block w-12 h-12 group rounded-lg overflow-hidden mx-auto"
                        >
                          <img
                            src={imageUrl}
...
function PageRecords({ savedRecords, onRefreshData }) {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return savedRecords.slice(1).filter((r) => {
      ...
      const searchMatch =
        (r[1] || "").toLowerCase().includes(lowerSearch) ||
  ...
  const handleDelete = async (serialNumber, imageUrl) => {
    ...
    try {
      ...
      setModal({
        type: "error",
        message: `${t("deleteFailed")}: ${e.message}`,
        onClose: () => setModal(null),
      });
    }
  };
    ...
    <div className="main-content rounded-2xl p-6 h-full flex flex-col bg-content">
      ...
      <div className="table-container flex-1 overflow-auto">
        {savedRecords.length <= 1 ? (
          <div className="text-center py-10">
            <p className="text-muted">Loading saved records...</p>
          </div>
        ) : (
                ...
                <tr key={record[0]} className="border-b border-main">
                  ...
                  <td className="p-3">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="relative block w-16 h-16 group rounded-lg overflow-hidden"
                    >
                      <img
                        src={record[5]}
                        className="w-full h-full object-cover"
...
function PageProfile({ admin, students, savedRecords }) {
  ...
  return (
    <div className="space-y-6">
      {/* Admin Profile Card */}
      <div className="main-content rounded-2xl p-6 bg-content">
        <h1 className="text-2xl khmer-title text-main mb-4">
          {t("adminProfile")}
        </h1>
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-lg text-theme">Admin</p>{" "}
            {/* Changed from Super Admin */}
          </div>
        </div>
      </div>
                ...
                <tr key={record[0]} className="border-b border-main">
                  ...
                  <td className="p-3">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="relative block w-12 h-12 group rounded-lg overflow-hidden"
                    >
                      <img
                        src={record[5]}
                        className="w-full h-full object-cover"
...
function PageSettings({ admin }) {
  ...
  // useEffect(() => { ... }, [admin.name]);
  // const isSuperAdmin = admin.name === SUPER_ADMIN_NAME;
  ...
  return (
    <div className="main-content rounded-2xl p-6 bg-content max-w-3xl mx-auto">
      <h1 className="text-2xl khmer-title text-main mb-6">
        {t("settingsTitle")}
      </h1>
              ...
              <div className="flex items-center p-1 rounded-lg bg-content">
                <button
                  ...
                  className={`px-3 py-1 rounded-md ${
                    theme === "light" ? "btn-theme text-white" : "text-muted"
                  }`}
                >
                  {t("light")}
                </button>
                <button
                  ...
                  className={`px-3 py-1 rounded-md ${
                    theme === "dark" ? "btn-theme text-white" : "text-muted"
                  }`}
                >
                  {t("dark")}
                </button>
              ...
              <div className="flex items-center space-x-2">
                {colors.map((c) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full focus:outline-none ring-2 ring-offset-2 ring-offset-content ${
                      color === c.class ? "ring-theme" : "ring-transparent"
                    }`}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center p-1 rounded-lg bg-content">
                <button
                  onClick={() => setLang("kh")}
                  className={`px-3 py-1 rounded-md ${
                    lang === "kh" ? "btn-theme text-white" : "text-muted"
                  }`}
                >
                  KH
                </button>
                <button
                  onClick={() => setLang("en")}
                  className={`px-3 py-1 rounded-md ${
                    lang === "en" ? "btn-theme text-white" : "text-muted"
                  }`}
                >
                  EN
                </button>
...
function MainApplication({
  ...
  return (
    <div className="relative min-h-screen md:flex">
      ...
      <div
        id="sidebar"
        className={`fixed top-0 left-0 w-64 h-full z-40 transform transition-transform duration-300 ease-in-out md:hidden ${
      ...
      <div
        id="content-wrapper"
        className="flex flex-col flex-1 w-full md:w-0 bg-main"
      >
        <Header
          admin={admin}
        ...
        <main className="w-full flex-1 p-4 md:p-8 overflow-y-auto">
          ...
          {currentPage === "scan" && (
            <PageScan students={students} onRefreshData={onRefreshData} />
          )}
          {currentPage === "list" && (
            <PageList students={students} savedRecords={savedRecords} />
          )}
          {currentPage === "records" && (
            <PageRecords
              savedRecords={savedRecords}
